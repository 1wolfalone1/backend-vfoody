...
public class ShopRejectOrderHandler : ICommandHandler<ShopRejectOrderCommand, Result>
{
    ...
    public async Task<Result<Result>> Handle(ShopRejectOrderCommand request, CancellationToken cancellationToken)
    {
        ...
        if (order.Status != (int)OrderStatus.Pending)
            throw new InvalidBusinessException($"Đơn hàng VFD{request.OrderId} đang trong trạng thái không thể từ chối");
        ...
        try
        {
            ...
            var messageNotification = string.Format(NotificationMessageConstants.Order_Reject_Content, order.Id);
